...
- (void)uploadGoodsImages:(EvaluationGoodsInfo *)info
{
    ...
    if (info.imageItems.count==4) {
        request.imageItems = info.imageItems;
    }else{
        [info.imageItems removeLastObject];
        request.imageItems=info.imageItems;
    }
    DZLog(@"上传商品%@ 图片....",info.goods_id);
    @weakify(self);
    [self.indicator startAnimationWithMessage:@"发布中"];
    [ESService fileUploadRequest:request success:^(NSArray *response) {
        DZLog(@"上传商品%@ 图片 完成.",info.goods_id);
        //保存返回的图片url数组
        info.image = response;
        [self doEvaluate];
